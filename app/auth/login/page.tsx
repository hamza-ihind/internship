  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!check2FAResponse.ok) {
        setError(check2FAData.error || 'Invalid email or password');
        setIsLoading(false);
        return;
      }
      ...
      if (check2FAData.requires2FA) {
        // Store password temporarily for completing sign in after 2FA
        sessionStorage.setItem('temp_password', password);

        router.push(
          `/auth/two-factor/verify?email=${encodeURIComponent(email)}&secret=${encodeURIComponent(check2FAData.encryptedSecret)}`,
        );
        return;
      }
